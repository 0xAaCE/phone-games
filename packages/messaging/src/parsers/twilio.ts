import { CreatePartyParams, DataOutput, IncomingMessage, IncomingMessageParser, JoinPartyParams, MessagePlatform, Output, ValidActions } from "../interfaces/parsers/index.js";
...
export class TwilioParser implements IncomingMessageParser<MessagePlatform.TWILIO> {
    ...
    async parse(message: IncomingMessage<MessagePlatform.TWILIO>): Promise<Output<ValidActions>> {
    ...
    private async parseAction(message: TwilioIncomingMessage): Promise<ValidActions> {
    ...
    private async parseDataOutput<T extends ValidActions>(action: T, message: TwilioIncomingMessage): Promise<DataOutput[T]> {
        switch (action) {
    ...
    private async parseCreatePartyParams(message: TwilioIncomingMessage): Promise<CreatePartyParams> {
        const text = message.Body;
        const [_action, gameName, partyName] = text.split(' ');
        return { gameName: gameName as ValidGameNames, partyName };
    }

    private async parseJoinPartyParams(message: TwilioIncomingMessage): Promise<JoinPartyParams> {
        const text = message.Body;
